...
<body>
                ...
                <div id="icon-color-section">
                    ...
                    <div class="icon-selector">
                        ...
                        <div id="selected-icon-display" style="margin-top: 12px; padding: 12px; background: var(--color-bg-light); border-radius: var(--radius-sm); text-align: center;">
                            <strong>Selected:</strong> <span id="selected-icon-name"></span>
                        </div>
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="start_date" value="<?= htmlspecialchars($med['start_date'] ?? date('Y-m-d')) ?>" required>
                        <small style="color: var(--color-text-secondary); display: block; margin-top: 4px;">
    ...
    <script>
        ...
        const selectedSecondaryColor = '<?= htmlspecialchars($med['secondary_color'] ?? '') ?>';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Update selected icon display initially
            const initialIconName = MedicationIcons.icons[selectedIcon]?.name || 'Pill/Tablet';
            document.getElementById('selected-icon-name').textContent = initialIconName;
            ...
            MedicationIcons.colors.forEach(color => {
                const div = document.createElement('div');
                div.className = 'secondary-color-option' + (color.value === selectedSecondaryColor ? ' selected' : '');
                div.dataset.color = color.value;
                div.title = color.name;
                div.style.backgroundColor = color.value;
                if (color.value === '#FFFFFF' || color.value === '#FFFFE0' || color.value === '#F5F5DC') {
